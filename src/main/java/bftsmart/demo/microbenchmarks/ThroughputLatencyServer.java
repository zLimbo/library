...
public final class ThroughputLatencyServer extends DefaultRecoverable {
    ...
    public ThroughputLatencyServer(int id, int interval, int replySize, int stateSize, boolean context, int signed,
            int write) {

        System.out.println("id:" + id + ", interval:" + interval + ", replySize:" + replySize + ", stateSize:"
                + stateSize + ", context:" + context + ", signed:" + signed + ", write:" + write);
    ...
    public byte[] execute(byte[] command, MessageContext msgCtx) {
        ...
        if (iterations % interval == 0) {
            ...
            System.out.println("Batch average size = " + batchSize.getAverage(false) + " (+/- "
                    + (long) batchSize.getDP(false) + ") requests");
            batchSize.reset();
    ...
    public static void main(String[] args) {
        if (args.length < 6) {
            System.out.println(
                    "Usage: ... ThroughputLatencyServer <processId> <measurement interval> <reply size> <state size> <context?> <nosig | default | ecdsa> [rwd | rw]");
            System.exit(-1);
        }
